    ...
    <div className={classes.gameCard}>
      <div className={classes.leftCol}>
        ...
        <div className={classes.info}>
          <h6 className={classes.baseGame}>{lang==='en'?'BASE GAME':'اللعبة الاساسية'}</h6>
      ...
      <div className={classes.rightCol}>
        <h4 className={classes.price}>{lang==='en'?`$${props.price}`:`${new Intl.NumberFormat("ar-EG").format(props.price)} US$`}</h4>
        <div className={classes.buttons}>
          <h6>{t('Moveto')} {props.moveTo}</h6>
          <h6>{t('Remove')}</h6>
        </div>
      </div>
    </div>
